...
export const CollaborativeWritingSequence = ({ 
  ...
  const [currentCharIndex, setCurrentCharIndex] = useState(0);
  const [showCursor, setShowCursor] = useState(true);
  const [activeCursorPosition, setActiveCursorPosition] = useState({ line: 0, char: 0 });
  ...
  const typeCurrentWriter = () => {
    ...
    if (currentCharIndex <= fullText.length) {
      ...
      setLines(newLines);
      setActiveCursorPosition({ line: currentWriterIndex, char: currentCharIndex });
...
export const CodeCommentEvolutionSequence = ({ 
  ...
  const codeSequence = [
    ...
    { 
      ...
      style: 'text-3xl font-mono font-semibold text-purple-700 dark:text-purple-300',
      prefix: '',
      typingSpeed: 60
    }
  ];
...
export const ManuscriptEditingSequence = ({ 
  ...
  const manuscriptSequence = [
    ...
    { 
      ...
      style: 'text-2xl font-serif text-gray-700 dark:text-gray-300',
      strikeStyle: 'text-2xl font-serif text-red-500 dark:text-red-400 line-through opacity-60',
      isEdit: true,
      pauseAfter: 2000
    },
    { 
    ...
    { 
      type: 'finalCall',
      text: `Let's build your space`, 
      style: 'text-3xl font-serif font-medium text-umber-800 dark:text-umber-100 underline decoration-2 underline-offset-4',
      typingSpeed: 70,
      pauseAfter: 2000
    }
  ];
  ...
  useEffect(() => {
    if (currentPhase < manuscriptSequence.length) {
      if (isTyping && !manuscriptSequence[currentPhase].isEdit && !manuscriptSequence[currentPhase].isEmphasis) {
        processCurrentPhase();
      } else if (manuscriptSequence[currentPhase].isEdit || manuscriptSequence[currentPhase].isEmphasis) {
        processCurrentPhase();
      }
    }
      ...
      return (
        <span>
          {parts.map((part, partIndex) => {
            if (partIndex === 1) {
              return <strong key={partIndex} className={line.boldStyle}>{part}</strong>;
            }
  ...
  const commands = [
    {
      ...
      output: [`✓ User ${userName} authenticated`, `✓ Destination: umber workspace`, `✓ Access granted`],
      delay: 800
    },
    {
    ...
    },
    {
      ...
      output: [`Directory created: your_space/`, `Changed directory to: your_space/`],
      delay: 500
    },
    {
  ...
  useEffect(() => {
    const runTerminalSequence = async () => {
      ...
      for (let i = 0; i < commands.length; i++) {
        ...
        // Add prompt line
        setLines(prev => [...prev, { type: 'prompt', text: '$ ', command: cmd.command, typing: true }]);
        setIsTyping(true);
      ...
      // Final prompt
      await sleep(500);
      setLines(prev => [...prev, { type: 'final', text: `${userName}@umber:~/your_space$ ready_to_build` }]);
    ...
    const typeCommand = async (command, commandIndex) => {
      for (let i = 0; i <= command.length; i++) {
        ...
        setLines(prev => prev.map((line, idx) => 
          idx === prev.length - 1 ? { ...line, currentText: command.slice(0, i) } : line
        ));
    ...
    if (line.type === 'final') {
      return (
        <div className="flex items-center text-green-400 font-bold">
          {line.text}
          {showCursor && <span className="bg-green-400 text-black ml-1 animate-pulse">█</span>}
        </div>
      );
    }
  };
